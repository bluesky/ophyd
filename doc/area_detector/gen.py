...
class DocRow(object):
    ...
    def __init__(self, row):
        ...
        self.record[0] = self.record[0].replace('ThresholdActualN', 'ThresholdActual$(N)')
    ...
    def multiple(self):
        ...
        if m:
            ...
            return [pv_base.replace('$(N)', str(n))
        ...
        if m:
            ...
            low = int(low)
            high = int(high)
            return [r0.replace(full_str, str(n))
    ...
    def python_source(self):
        ...
        return [get_doc(rec, type_) for rec, type_ in zip(self.record, self.record_type)]
