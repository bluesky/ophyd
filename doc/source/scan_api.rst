...
Intermediate Scans: An "Algebra" for Scan Specification
-------------------------------------------------------

   * StepScanner(start, stop, num, step=None)
   * TimeScanner(delay, num, duration=None)
   * FlyScanner(start, stop, velocity)
   * BlockingRead()
   * ConcurrentRead()

A D2 scan is an "inner product" of two step scans.::

   step1 = StepScanner(0, 5, 5)
   step1
   step2 = StepScanner(0, 50, 5)
   step2
   my_d2_scan = step1 + step2
   my_d2_scan

A mesh scan is an "outer product."::

   my_mesh_scan = step1 * step2
   my_mesh_scan

Advanced Scans: Step-by-Step Scan Instructions
----------------------------------------------

It will also be possible to specify scans in the most general way, giving
step by step instructions. Each instruction gives an object, the name of a
method on that object, any positional arguments, and any keyword arguments.

Example::

[(motor1, 'move', (5,), {}), (motor1, 'wait', (), {}), (det1, 'read', (), {})]

Note that these instructions can be generated dynamically during the scan.
