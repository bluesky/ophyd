...
class MultiStatePathOrchestror():
    '''
    ...
    These 'paths' are calculated based on the optional 'neighbours' dictionary, which for each 
    ...
    components : Dictionary.
        ...
        be created so that they can be accessed using 'collection'.'name' for each component. Components 
        can be any components including but not limited too: motor axes, detectors, gate valves (and
        their configuration attributes) or even another PreDefinedPositions 'collection'.
    ...
    def __init__(self, components, *,locations={}, neighbours={}, regions={}):
        ...
        #define the dictionaries used for real 'components' (access to these will be made when setting up
        #the pseudostatepositioner, and will be by the syntax self.real.components etc. 
        _real_components = {}
        _real_locations = {}
        ...
        for attribute in self.components:
            #top level access to all components, to 'hide' the real/orchestrator abstraction from users.
            ...
            if isinstance(components[attribute],MultiStatePathOrchestrator):
                self.orchestrator.components[attribute] = self.components[attribute]
            else:
                _real_components[attribute] = self.components[attribute]

        #seperate the locations and regions dictionary into orchestrator and real parts.
        for location in locations:
            ...
            for i in range(0, len(locations[location]), 2):
                if locations[location][i] in _real_components:
                    _real_locations[location].extend([locations[location][i],locations[location][i+1]])
                    try:
                    ...
                    self.orchestrator.locations[location].extend([locations[location][i],
        ...
        self.pseudo = PseudoStatePositioner(locations=_real_locations, neighbours=_real_neighbours, 
        ...
        def mv_plan(to_location):
            ...
            if value not in list(self.locations.keys()):
                raise ValueError('{} is not a pre defined position'.format(location))  
            else:
                for location in path_list:
                    print ('Move {} to "{}"'.format(self.name,location))
                    for component in self.orchestrator.location:
                        try:
                            yield from getattr(getattr(self, component), location)
    ...
    def find_path(self,from_location=None,to_location=None):
        ...
        -------
        path_list: list.
            A list of locations indicating the path to take to reach the required position.
        '''
